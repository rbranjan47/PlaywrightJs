...
// })
test("Orange HRM Tests", async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Save' }).click();
    try {
        expect(await page.locator("//div[@role='alertdialog']//p").textContent()).toBe('SuccessSuccessfully Saved');
        expect(await page.locator("//div[@class='oxd-table-card']//div[contains(text(),'JohnSnow001')]").textContent()).toBe('JohnSnow001');
        expect(await page.locator("//div[@class='oxd-table-card']//div[contains(text(),'Peter Mac Anderson')]").textContent()).toBe('Peter Mac Anderson');
        expect(await page.locator("//div[@class='oxd-table-card']//div[contains(text(),'Admin')]").textContent()).toBe('Admin');
    }
    catch (errors) {
        console.log(errors);
    }
